...
class KiwoomWebSocketProxy:
    ...
    async def connect_to_kiwoom(self, client_ws: WebSocket):
        try:
            logging.info("Kiwoom WebSocket 서버에 연결을 시도합니다...")
            self.kiwoom_ws = await websockets.connect(SOCKET_URL)
            logging.info("Kiwoom WebSocket 서버에 연결되었습니다.")
            return True
        except Exception as e:
            logging.error(f"Kiwoom WebSocket 서버 연결 실패: {e}")
            await client_ws.send_json({"status": "error", "detail": f"Kiwoom 연결 실패: {e}"})
            return False

    async def login_to_kiwoom(self, client_ws: WebSocket):
        login_param = {
            'trnm': 'LOGIN',
            'token': ACCESS_TOKEN
        }
        logging.info(f"Kiwoom 로그인 요청: {ACCESS_TOKEN}")
        ...
        if login_response.get('trnm') == 'LOGIN' and login_response.get('return_code') == 0:
            ...
            await client_ws.send_json({"status": "Kiwoom 로그인 성공"})
            return True
        else:
            msg = login_response.get("return_msg", "알 수 없는 로그인 오류")
            logging.error(f'Kiwoom 로그인 실패하였습니다: {msg}')
            await client_ws.send_json({"status": "Kiwoom 로그인 실패", "detail": msg})
            return False

    async def _kiwoom_to_client_task(self, client_ws: WebSocket):
        while self.is_running:
            try:
                ...
                try:
                    ...
                    logging.error(f"실시간 체결 정보 파싱 오류: {e}")

                await client_ws.send_text(message)
            except websockets.ConnectionClosed:
                logging.warning("Kiwoom WebSocket 연결이 닫혔습니다.")
                break
            except Exception as e:
                if self.is_running:
                    logging.error(f"Kiwoom에서 클라이언트로 전달 중 오류 발생: {e}")
                break
        self.is_running = False

    async def _client_to_kiwoom_task(self, client_ws: WebSocket):
        while self.is_running:
            try:
                message_text = await client_ws.receive_text()
                logging.info(f"클라이언트로부터 메시지 수신: {message_text}")
                ...
                try:
                    ...
                    if action == 'subscribe' and stock_code:
                        ...
                        logging.info(f"Kiwoom으로 구독 메시지 전송: {subscription_msg}")
                        ...
                        logging.info(f"Kiwoom으로 구독 해지 메시지 전송: {unsub_msg}")

                    else:
                        logging.warning(f"알 수 없는 액션 또는 잘못된 메시지 형식: {client_data}")

                except json.JSONDecodeError:
                    logging.warning(f"클라이언트로부터 받은 메시지가 JSON 형식이 아님: {message_text}")

            except WebSocketDisconnect:
                logging.info("클라이언트가 FastAPI WebSocket에서 연결을 끊었습니다.")
                break
            except Exception as e:
                if self.is_running:
                    logging.error(f"클라이언트에서 Kiwoom으로 전달 중 오류 발생: {e}")
                break
        self.is_running = False

    async def run(self, client_ws: WebSocket):
        if not await self.connect_to_kiwoom(client_ws):
            return

        if not await self.login_to_kiwoom(client_ws):
            await self.disconnect()
            return
            
        self.is_running = True
        
        kiwoom_reader = asyncio.create_task(self._kiwoom_to_client_task(client_ws))
        client_reader = asyncio.create_task(self._client_to_kiwoom_task(client_ws))
    ...
    async def disconnect(self):
        ...
        if self.kiwoom_ws and self.kiwoom_ws.state != State.CLOSED:
            if self.registered_groups:
                logging.info(f"저장된 모든 그룹({self.registered_groups})에 대해 구독 해지를 시도합니다.")
                for grp_no in list(self.registered_groups):
                    ...
                    try:
                        ...
                        logging.info(f"그룹 {grp_no} 구독 해지 메시지 전송 완료.")
                    except Exception as e:
                        logging.error(f"그룹 {grp_no} 구독 해지 메시지 전송 중 오류 발생: {e}")
            
            await self.kiwoom_ws.close()
            logging.info("Kiwoom WebSocket 서버와 연결이 끊어졌습니다.")
        logging.info("프록시 작업이 종료되었습니다.")


@router.websocket("/ws/trade-price")
async def websocket_trade_price(websocket: WebSocket):
    await websocket.accept()
    proxy = KiwoomWebSocketProxy()
    try:
        await proxy.run(websocket)
    except Exception as e:
        # 프록시 실행 중 예기치 않은 오류 기록
        logging.error(f"WebSocket 프록시에서 예기치 않은 오류 발생: {e}")
    finally:
        # 클라이언트 연결을 정상적으로 닫도록 보장
        logging.info("클라이언트와의 FastAPI WebSocket 연결이 닫혔습니다.")
